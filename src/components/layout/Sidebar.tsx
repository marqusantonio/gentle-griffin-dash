      ...
      <aside className={`w-64 fixed top-0 bottom-0 left-0 z-50 flex flex-col liquid-glass p-4 transition-transform duration-300 md:translate-x-0 ${
        ...
        <div className="flex items-center justify-between px-2 mb-3">
          ...
          >
            ...
            >
              ...
              <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-[#10b981] border-2 border-[#050512] animate-ping" />
            </div>
            <div>
          ...
          {mainNavItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl font-medium text-xs transition-all duration-200 group relative ${
                  isActive
                    ? 'liquid-glass border-white/35 text-white shadow-[0_8px_25px_rgba(0,0,0,0.6)] scale-[1.02]'
                    : 'text-[#9494b8] hover:text-white hover:bg-white/[0.08] border border-transparent'
                }`}
              >
                {isActive && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 rounded-r-full bg-gradient-to-b from-[#ff2d95] to-[#00e5ff] shadow-[0_0_15px_#00e5ff]" />
                )}
                <Icon 
                  className={`w-4 h-4 transition-transform duration-300 group-hover:scale-120 ${
              ...
              <div className="space-y-1.5">
                {onlineProfiles.slice(0, 5).map((rec: UserProfile) => {
                  ...
                  return (
                    <div
                      key={rec.id}
                      className="flex items-center justify-between p-2 rounded-2xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/10 transition-all text-xs shadow-sm hover:border-[#00e5ff]/40"
                    >
                      <div 
                      ...
                      >
                        <div 
                          className="w-7 h-7 rounded-full flex items-center justify-center text-slate-900 text-xs font-bold shrink-0 border border-white/30"
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-xl text-[10px] font-bold transition-transform hover:scale-110 shrink-0 ml-1 shadow-sm ${
                          following
        ...
        <div className="pt-3 border-t border-white/10 space-y-2">
          ...
          <div 
            ...
            className="flex items-center gap-2.5 p-2.5 rounded-2xl liquid-glass-pill cursor-pointer transition-all group hover:border-[#00e5ff]/50 hover:scale-102"
          >
            <div 
              className="w-9 h-9 rounded-full flex items-center justify-center text-slate-900 font-bold text-xs shadow-lg shrink-0 border border-white/40"
