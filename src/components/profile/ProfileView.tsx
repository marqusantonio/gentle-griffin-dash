...
export const ProfileView: React.FC = () => {
  const { currentUser, updateCurrentUser, posts } = useWevids();
        ...
        <div className="px-6 pb-6 pt-2 relative">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 -mt-16 mb-4">
            ...
            <div className="flex items-center gap-4">
              <div className="text-center p-2 rounded-xl bg-white/5 border border-white/5">
                <div className="font-orbitron font-bold text-sm text-[#00e5ff]">{currentUser.followers.toLocaleString()}</div>
                <div className="text-[10px] text-[#8a8aa8]">Followers</div>
              </div>
              <div className="text-center p-2 rounded-xl bg-white/5 border border-white/5">
                <div className="font-orbitron font-bold text-sm text-[#ff2d95]">{currentUser.likes.toLocaleString()}</div>
              ...
              <div className="text-center p-2 rounded-xl bg-white/5 border border-white/5">
                ...
                <div className="text-[10px] text-[#8a8aa8]">Tipping Pool</div>
              </div>
            </div>
          </div>
          ...
          {!isEditing ? (
            <div className="space-y-2">
              ...
              <div className="text-xs text-[#8a8aa8]">{currentUser.handle} · {currentUser.location}</div>
            ...
            <form onSubmit={handleSave} className="space-y-3 pt-2 text-xs">
              ...
              <div>
                <label className="text-[#8a8aa8] font-bold block mb-1">Location</label>
                <input
                  type="text"
                  value={location}
              ...
              <button
                type="submit"
                className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#ff2d95] to-[#00e5ff] text-slate-900 font-orbitron font-bold"
              >
                Save Changes
              </button>
            </form>
