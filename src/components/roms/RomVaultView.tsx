...
export const RomVaultView: React.FC = () => {
  const { roms, addRom, currentUser } = useWevids();
  ...
  const handleCreateRom = (e: React.FormEvent) => {
    ...
    addRom({
      ...
      romType,
      status: 'Official',
      maintainer: `${currentUser.name} (${currentUser.handle})`,
      maintainerHandle: currentUser.handle,
      version,
      ...
      changelog: changelog ? changelog.split('\n') : ['Initial release build', 'Optimized kernel schedulers']
    });
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="p-6 rounded-3xl liquid-glass border border-white/10 relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#8a8aa8] mt-1 max-w-xl">
            Verified ROM packages, overclocked gaming kernels, TWRP recovery scripts, and Magisk modules with integrity verification.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              placeholder="Search device, ROM or maintainer..."
      ...
      {filteredRoms.length === 0 ? (
        <div className="liquid-glass rounded-3xl p-12 border border-white/10 text-center space-y-3 shadow-xl">
          <Cpu className="w-12 h-12 text-[#00e5ff] mx-auto opacity-40 animate-pulse" />
          <h3 className="font-orbitron font-bold text-base text-white">No ROM packages submitted yet</h3>
          <p className="text-xs text-[#8a8aa8] max-w-sm mx-auto">
            Click "Submit ROM Build" to publish a flashable ZIP with SHA256 checksums to the vault.
          </p>
        </div>
      ) : (
            ...
            <form onSubmit={handleCreateRom} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. HyperOS 2.0 Fastboot ROM"
