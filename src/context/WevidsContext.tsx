...
export const WevidsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const syncWithSupabase = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // If clips table is populated, use it. Otherwise, fallback to video posts from posts table
      if (clipsRes.data && clipsRes.data.length > 0) {
        dispatch({ type: 'SET_CLIPS', payload: clipsRes.data });
      } else if (postsRes.data) {
        const videoClipsFromPosts: ShortClipItem[] = postsRes.data
          .filter((p: any) => p.mediaType === 'video' || p.video_url || (p.mediaUrl && p.mediaUrl.endsWith('.mp4')))
          .map((p: any) => ({
            id: p.id,
            userId: p.userId || p.user_id || 'guest',
            title: p.title || p.caption || p.content || 'Creator Video Clip',
            description: p.content || p.caption || '',
            videoUrl: p.mediaUrl || p.video_url || '',
        ...
        if (videoClipsFromPosts.length > 0) {
          dispatch({ type: 'SET_CLIPS', payload: videoClipsFromPosts });
        }
      }
  ...
  const addClip = async (clip: ShortClipItem) => {
    ...
    if (isSupabaseConfigured()) {
      try {
        const { error } = await supabase.from('clips').upsert(clip);
        if (error) {
          // If clips table does not exist, store it directly in posts table so it never fails!
          await insertPostWithAutoFallback({
            id: clip.id,
            userId: clip.userId,
        ...
        toast.success('Clip saved locally!');
      }
    }
  };
  ...
  const toggleClipLike = async (clipId: string) => {
    ...
    if (isSupabaseConfigured()) {
      try {
        const { error } = await supabase.from('clips').update({ likes: newLikesCount }).eq('id', clipId);
        if (error) {
          await supabase.from('posts').update({ likes: newLikesCount }).eq('id', clipId);
        }
      } catch {
        // Safe offline
      }
  ...
  const addClipComment = async (clipId: string, comment: any) => {
    ...
    if (isSupabaseConfigured() && clip) {
      try {
        ...
        const { error } = await supabase.from('clips').update({ comments: updatedComments }).eq('id', clipId);
        if (error) {
          await supabase.from('posts').update({ comments: updatedComments }).eq('id', clipId);
        }
      } catch {
        // Safe offline
      }
...
<dyad-write path="src/context/WevidsContext.tsx" description="Completing the full WevidsContext provider with fallback and action handlers">
import React, { createContext, useContext, useReducer, useEffect, ReactNode, useCallback } from 'react';
import { wevidsReducer, initialWevidsState, WevidsState } from './wevidsReducer';
import { 
  PostItem, 
...
export const WevidsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const toggleClipDislike = async (clipId: string) => {
    ...
    if (isSupabaseConfigured()) {
      try {
        const { error } = await supabase.from('clips').update({ dislikes: newDislikesCount }).eq('id', clipId);
        if (error) {
          await supabase.from('posts').update({ dislikes: newDislikesCount }).eq('id', clipId);
        }
      } catch {
        // Safe offline
      }
